#include <functional>
#include <utility>
#include <vulkan/vulkan.h>

template < typename T >
class VDeleter
{
public:
    VDeleter()
    {
        this->deleter = []( T obj ) {};
    }

    VDeleter( std::function< void( T, VkAllocationCallbacks * ) > deletef )
    {
        this->deleter = [=]( T obj ) { deletef( obj, nullptr ); };
    }

    VDeleter(
        VkInstance instance,
        std::function< void( VkInstance, T, VkAllocationCallbacks * ) >
            deletef )
    {
        this->deleter = [instance, deletef]( T obj ) {
            deletef( instance, obj, nullptr );
        };
    }

    VDeleter(
        VkDevice device,
        std::function< void( VkDevice, T, VkAllocationCallbacks * ) > deletef )
    {
        this->deleter = [device, deletef]( T obj ) {
            deletef( device, obj, nullptr );
        };
    }

    VDeleter( VDeleter const & ) = delete;
    VDeleter( VDeleter &&right )
    {
        ( *this ) = std::move( right );
    }

    ~VDeleter()
    {
        cleanup();
    }

    T *replace()
    {
        cleanup();
        return &object;
    }

    T operator*() const
    {
        return object;
    }

    VDeleter &operator=( T rhs )
    {
        if( rhs != object )
        {
            cleanup();
            object = rhs;
        }
        return *this;
    }
    VDeleter &operator=( VDeleter const & ) = delete;
    VDeleter &operator=( VDeleter &&right )
    {
        if( this != &right )
        {
            cleanup();
            object = right.object;
            right.object = VK_NULL_HANDLE;
            deleter = std::move( right.deleter );
            right.deleter = []( T obj ) {};
        }
        return *this;
    }

    template < typename V >
    bool operator==( V rhs )
    {
        return object == T( rhs );
    }
    template < typename V >
    bool operator!=( V rhs )
    {
        return !( *this == rhs );
    }

private:
    T object{VK_NULL_HANDLE};
    std::function< void( T ) > deleter;

    void cleanup()
    {
        if( object != VK_NULL_HANDLE )
        {
            deleter( object );
            object = VK_NULL_HANDLE;
        }
    }
};
